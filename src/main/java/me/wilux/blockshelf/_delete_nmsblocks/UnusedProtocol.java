...
*/
        ...
        Main.protocolManager.addPacketListener(
                new PacketAdapter(Main.plugin, ListenerPriority.NORMAL,PacketType.Play.Client.USE_ITEM) {
                    @Override
                    public void onPacketReceiving(PacketEvent event) {
                        if (event.getPacketType() == PacketType.Play.Client.USE_ITEM) printAllEventProperties(event);
                    }});
        Main.protocolManager.addPacketListener(
                new PacketAdapter(Main.plugin, ListenerPriority.NORMAL,PacketType.Play.Client.RECIPE_DISPLAYED) {
                    @Override
                    public void onPacketReceiving(PacketEvent event) {
    ...
    public static void printAllEventProperties(PacketEvent event){
        ...
        Main.logger.warning("EVT "+event.getPacketType());
    ...
    static <T> void lazyTry(PacketContainer pack,Function<PacketContainer, StructureModifier<T>> func, String name){
        try{
            func.apply(pack).getValues().stream().forEach(
                    x->Main.logger.warning("    "+name+"() "+atMaxLength(x.toString())));
        } catch (Exception e){
            Main.logger.severe("ERROR For:"+name);
        }
    }
